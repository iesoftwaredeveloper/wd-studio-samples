...
<xsl:stylesheet
    ...
    <xsl:template match="je:Report_Entry" mode="new_line">
        <xsl:variable name="debit_balance">
            <xsl:value-of select="xs:decimal(je:Debit_Amount) - xs:decimal(je:Credit_Amount)"/>
        </xsl:variable>
        <xsl:variable name="credit_balance">
            <xsl:value-of select="xs:decimal(je:Credit_Amount) - xs:decimal(je:Debit_Amount)"/>
        </xsl:variable>
        <xsl:if test="$debit_balance != 0 or $credit_balance != 0">
            <bsvc:Journal_Entry_Line_Replacement_Data>
                ...
                <bsvc:Memo>
                    <xsl:value-of select="'Balance Migration'"/>
                </bsvc:Memo>
                <bsvc:External_Reference_ID>
                ...
                <xsl:if test="string-length(je:Customer) != 0">
                    <bsvc:Worktags_Reference>
                        <bsvc:ID bsvc:type="Customer_ID">
    ...
    <xsl:template match="je:Report_Entry" mode="offset">
        ...
        <xsl:if test="$debit_balance != 0 or $credit_balance != 0">
            <bsvc:Journal_Entry_Line_Replacement_Data>
                ...
                <bsvc:Memo>
                    <xsl:value-of select="'Balance Migration BU Offset'"/>
                </bsvc:Memo>
                <bsvc:External_Reference_ID>
                ...
                <xsl:if test="string-length(je:Business_Unit) != 0">
                    <bsvc:Worktags_Reference>
                        <bsvc:ID bsvc:type="Organization_Reference_ID">
                            <xsl:value-of select="je:Business_Unit/je:ID[@je:type = 'Organization_Reference_ID']"/>
                        </bsvc:ID>
                    </bsvc:Worktags_Reference>
                </xsl:if>
                <xsl:if test="string-length(je:Region) != 0">
